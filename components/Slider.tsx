...
const Slider = ({
  ...
  return (
    <div className="flex flex-col gap-y-2">
      {sliderhead && (
        <span className="relative max-w-fit rounded-full bg-white px-5 py-2 font-bold text-blue-950 shadow-md tb:py-1">
          {sliderhead}
        </span>
      )}
        ...
        <SplideTrack className="py-4">
          {list.map(({ description, imgLink }) => (
            ...
            >
              ...
              {subtitle && (
                <span className="whitespace-pre text-center text-xs font-medium capitalize dt:text-sm">
                  {description}
                </span>
              )}
            </SplideSlide>

...
//issue #1: required to get notfound if the folder router is not equal
            <div className="group relative z-20">
                    ...
                    <div className="hidden group-hover:block group-focus:visible grow">
                        <div className="absolute top-full flex w-fit flex-col overflow-hidden rounded pt-3 shadow-md grow">
                            {categories?.map(
                                ({ id, category }: CategoryProps) => (
                                    <Link
                                        key={id}
                                        href={`${process.env.CURRENT_DOMAIN}/${id}`}
